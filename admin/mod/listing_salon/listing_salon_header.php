...
if (isset($_GET['id_suppr'])) {
    ...
    sql_simple_delete('salon', $id_suppr);
    header('location: index.php?page=listing_salon');
}

$title_page = 'Listing des salons';

// Etape 1 : Ecriture de la requete SQL
$sql = "SELECT id, name FROM salon";
...
$html .= '   <a href="index.php?page=add_salon"><img src="pic/interface/add_salon.png"></a>';
$html .= '</div>';
...
if ($rs && mysqli_num_rows($rs)) {
    ...
    $html .= '       <td class="tab_td">Nom salon</td>';
    ...
    while ($data = mysqli_fetch_assoc($rs)) {
        ...
        if ($_SESSION[SITE_NAME]['id_salon'] != $data['id']) {
            $html .= '            <a onclick="if(window.confirm(\'Etes vous sur ?\')) return true; else return false;" href="index.php?page=listing_salon&id_suppr=' . $data['id'] . '">';
            $html .= '                <img src="pic/interface/suppr.png">';
            $html .= '            </a>';
        }
        $html .= '            <a href="index.php?page=add_salon&id=' . $data['id'] . '">';
